...
const App = () => {
  const estiloButao = {marginTop: 12, paddingTop: 8, paddingBottom: 8,backgroundColor: 'blueviolet', color: 'white', border: 'none', width: "100%",borderRadius: 8}
  const classRotulo = 'Nome'
  const textoButao = () =>  'Enviar' 
  return(
    <div
      style={{margin: 'auto', width: 768, backgroundColor: '#EEE', padding: 12, borderRadius: 8}}>
           <label className="rotulo" htmlFor="nome" style={{display: 'block',marginBottom: 4}}>Nome:</label>
            ...
            <button onClick={} style={estiloButao}>{textoButao()}</button>
     </div>
  )
}
